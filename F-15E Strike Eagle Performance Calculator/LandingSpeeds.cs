...
{
    private static readonly string livDbLoc =
        Worker.ReplaceExtraSlashes(AppDomain.CurrentDomain.BaseDirectory + "\\F15EPerformance.db");

    private static readonly string connectionString = "Data Source = " + livDbLoc + ";";
    ...
    public (double FlapDownSpeed, double FlapUpSpeed) GetLandingSpeed(int weight)
    {
        using (var connection = new SQLiteConnection(connectionString))
        {
            ...
            {
                command.CommandText =
                    "SELECT Weight, FlapDown, FlapUp FROM LandingSpeeds WHERE Weight <= @Weight ORDER BY Weight DESC LIMIT 2";
                ...
                using (var reader = command.ExecuteReader())
                {
                    if (reader.Read())
                    {
                        ...
                        if (reader.Read())
                        {
                            ...
                            var interpolatedFlapDownSpeed = lowerFlapDownSpeed +
                                                            interpolationFactor *
                                                            (upperFlapDownSpeed - lowerFlapDownSpeed);
                            var interpolatedFlapUpSpeed = lowerFlapUpSpeed +
                                                          interpolationFactor * (upperFlapUpSpeed - lowerFlapUpSpeed);
